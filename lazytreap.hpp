#ifndef CPPCP_LAZYTREAP
#define CPPCP_LAZYTREAP
...
template <typename Val, typename Lazy, bool is_reversible> struct LazyNode {
    Val val, cum_val;
    Lazy lazy;
    LazyNode* left;
    LazyNode* right;
    ...
    LazyNode(const Val& _val, const Lazy& _lazy)
        : val(_val),
          cum_val(_val),
          lazy(_lazy),
          left(nullptr),
...
template <
    ...
    typename Allocator = DynamicAllocator<
        Treap::LazyNode<Val, Lazy, is_reversible>>>
    requires std::is_invocable_r_v<Val, ValOp, Val, Val>
             ...
             && std::is_invocable_r_v<Val, Apply, Val, Lazy, usize>
             && is_node_allocator<
                 Allocator,
                 Treap::LazyNode<Val, Lazy, is_reversible>,
                 Val,
...
public:
    using Node = Treap::LazyNode<Val, Lazy, is_reversible>;

    LazyImplicitTreap(const Lazy& nil_lazy = Lazy())
        : root(nullptr),
          lazy_nil(nil_lazy) {}
    ...
    LazyImplicitTreap(const T& source, const Lazy& nil_lazy = Lazy())
        : root(nullptr),
          lazy_nil(nil_lazy) {
    ...
    LazyImplicitTreap(const LazyImplicitTreap& other) : root(nullptr) {
        walk_inorder(other.root, [&](const Node* node) {
            root = merge(root, allocator.alloc(node->val));
        });
    }

    LazyImplicitTreap& operator=(const LazyImplicitTreap& other) {
        if (this == &other) {
            return *this;
        }
        if (root != nullptr) {
    ...
    void update(const usize pos, const Val& val) {
        ...
        target->val = val_op(target->val, val);
        update(target);

        root = merge(merge(left, target), right);
    }

    void update(const usize l_pos, const usize r_pos, const Lazy& lazy) {
        debug_assert(l_pos <= r_pos, "trying to update a segment of size < 0");
        debug_assert(
            r_pos < size(), "r_pos for update is past end of structure"
        );

        const auto [left, rest] = split(root, l_pos);
        const auto [target, right] = split(rest, r_pos - l_pos + 1);

        target->lazy = lazy_op(target->lazy, lazy);
        propagate(target);
    ...
    void insert(const usize pos, const Val& val) {
        ...
        const auto target = allocator.alloc(val, lazy_nil);
        root = merge(merge(left, target), right);
    }
    ...
    std::pair<LazyImplicitTreap, LazyImplicitTreap> split(const usize left_size
    ) {
        debug_assert(
        ...
        root = nullptr;
        return {LazyImplicitTreap(left), LazyImplicitTreap(right)};
    }

    void join(const LazyImplicitTreap& other) {
...
private:
    ...
    LazyImplicitTreap(Node* node) : root(node) {}
    ...
    void propagate(Node* node) const {
        ...
        for (const auto& child : {node->left, node->right}) {
            if (child != nullptr) {
                child->lazy = lazy_op(child->lazy, node->lazy);
            }
        }
        node->val = apply(node->val, node->lazy, 1);
        node->cum_val = apply(node->cum_val, node->lazy, node->size);
        node->lazy = lazy_nil;
    }
    ...
    void update(Node* node) const {
        ...
        for (const auto& child : {node->left, node->right}) {
            if (child != nullptr) {
                propagate(child);
                node->cum_val = val_op(node->cum_val, child->cum_val);
